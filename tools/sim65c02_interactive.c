...
static void do_adc(CPU *c,uint8_t v){uint16_t r=c->A+v+c->C;c->V=(~(c->A^v)&(c->A^r)&0x80)?1:0;if(!c->D){c->C=(r>0xFF)?1:0;c->A=r&0xFF;set_nz(c,c->A);return;}int lo=(c->A&0x0F)+(v&0x0F)+c->C;int hi=(c->A>>4)+(v>>4);if(lo>9){lo+=6;hi+=1;}if(hi>9){hi+=6;}c->C=(hi>0x0F)?1:0;c->A=(uint8_t)(((hi<<4)|(lo&0x0F))&0xFF);set_nz(c,c->A);}
