/*
 * asm65c02.c  —  Two-pass Toy 65C02 assembler  (v1.8, Mar 2026)
 *
 ...
 * v1.8: Fix explicit accumulator operands (INC A/DEC A/etc.), bare hex
 *       zero-page sizing, and address-space overflow diagnostics.
 *
 * Build (standalone):
 ...
 *   v1.8  (Mar 2026)  Fix explicit accumulator operands, bare hex zero-page
 *                     sizing, and 64K address-space overflow diagnostics.
 */
...
static Operand parse_operand(const char *raw_op, const char *mn, int pc, int pass2) {
    ...
    /* explicit accumulator operand: INC A, DEC A, ASL A, etc. */
...
static void listing_set_bytes(ListingRecord *rec, int start, const uint8_t *bytes, int count) {
...
static int note_mem_write(int lineno, int addr) {
    if (addr < 0 || addr > 0xFFFF) {
        if (!address_overflow) {
            char msg[ERR_LEN];
            snprintf(msg, ERR_LEN, "Address $%05X exceeds 64K address space", addr);
            add_error(lineno, msg);
        }
        address_overflow = 1;
        return 0;
...
static void add_overflow_error(int lineno, int addr) {
    address_overflow = 1;
    char msg[ERR_LEN];
    snprintf(msg, ERR_LEN, "Address $%05X exceeds 64K address space", addr);
    add_error(lineno, msg);
...
static void check_emit_range(int lineno, int start, int count) {
    if (count <= 0) return;
    if (!addr_in_range(start)) {
        add_overflow_error(lineno, start);
        return;
    }
    if (!addr_in_range(start + count - 1))
        add_overflow_error(lineno, start + count - 1);
}

static void note_code_addr(int addr) {
    if (!addr_in_range(addr)) return;
...
static int assemble(const char *source) {
    ...
    nsyms = 0; nerrors = 0; nlines = 0; nlisting = 0; address_overflow = 0;
    g_scope[0] = '\0';
    ...
    for (int li = 0; li < nl; li++) {
        ...
        /* directives */
        if (!strcmp(mn, ".org")) {
            int e=0; pc = eval_expr(operand, pc, 0, &e);
            if (!e && !addr_in_range(pc)) add_overflow_error(lineno, pc);
            info->pc = pc; continue;
        }
        if (!strcmp(mn, ".res")) {
            int e=0; int cnt = eval_expr(operand, pc, 0, &e);
            if (!e) check_emit_range(lineno, pc, cnt);
            pc += cnt; continue;
        }
        if (!strcmp(mn, ".byte")) {
            uint8_t tmp[LINE_LEN]; int n = parse_dot_byte(operand, pc, 0, tmp, LINE_LEN, lineno);
            check_emit_range(lineno, pc, n);
            pc += n; continue;
        }
        if (!strcmp(mn, ".word")) {
            ...
            if (!operand[0]) cnt = 0;
            check_emit_range(lineno, pc, cnt * 2);
            pc += cnt * 2; continue;
        }
        ...
        if (mnem_known(mn)) {
            ...
            int sz = instr_size(mn, op.mode);
            check_emit_range(lineno, pc, sz);
            pc += sz;
        } else {
            char msg[ERR_LEN];
    ...
    for (int li = 0; li < nlines; li++) {
        ...
        if (!strcmp(mn, ".byte")) {
            ...
            for (int i = 0; i < n; i++) {
                int addr = pc + i;
                if (note_mem_write(lineno, addr))
                    mem[addr] = tmp[i];
                note_code_addr(addr);
            }
            listing_set_bytes(lrec, pc, tmp, n);
            continue;
        }
        if (!strcmp(mn, ".word")) {
            const char *q = op;
            ...
            while (*q) {
                ...
                if (expr[0]) {
                    int e=0; int raw_val=eval_expr(expr,wpc,1,&e);
                    int val=raw_val&0xFFFF;
                    if (e) { char msg[ERR_LEN]; snprintf(msg,ERR_LEN,".word '%s': undef",expr); add_error(lineno,msg); }
                    if (!e && !addr_in_range(raw_val)) {
                        char msg[ERR_LEN];
                        snprintf(msg, ERR_LEN, ".word '%s': value $%05X exceeds 16 bits", expr, raw_val);
                        add_error(lineno, msg);
                    }
                    if (note_mem_write(lineno, wpc))
                        mem[wpc] = val & 0xFF;
                    if (note_mem_write(lineno, wpc+1))
                        mem[wpc+1] = (val >> 8) & 0xFF;
                    note_code_addr(wpc);
                    note_code_addr(wpc+1);
                    wpc += 2;
                }
                if (*q == ',') q++;
            }
            {
                ...
                for (int i = 0; i < count; i++) {
                    int addr = pc + i;
                    tmp[i] = addr_in_range(addr) ? mem[addr] : 0;
                }
                listing_set_bytes(lrec, pc, tmp, count);
            }
            continue;
        }
        if (!strcmp(mn,".opt")||!strcmp(mn,".setcpu")||
        ...
        if (sz >= 2) {
            if (m == M_REL) {
                ...
                ibuf[1] = (uint8_t)(offset & 0xFF);
            } else {
                ibuf[1] = (uint8_t)(val & 0xFF);
                if (sz == 3) ibuf[2] = (uint8_t)((val >> 8) & 0xFF);
            }
        }
        for (int i = 0; i < sz; i++) {
            int addr = pc + i;
        ...
        listing_set_bytes(lrec, pc, ibuf, sz);
    }
...
static void size_report(void) {
    if (address_overflow) {
        printf("\nROM footprint: address-space overflow; footprint is invalid.\n");
        return;
    }
...
static void asm_usage(FILE *out) {
    fprintf(out,
        "asm65c02 v1.8 — Toy 65C02/6502 two-pass assembler\n"
        "\n"
