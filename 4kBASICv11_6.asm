; =============================================================================
; 4K INTEGER BASIC  v11.6  ?  65C02  ?  Kowalski Simulator
; Pre-loaded program: feature showcase + Mandelbrot (type RUN to execute, NEW to clear)
; =============================================================================
; Copyright Vincent Crabtree, Mar 2026.
; Use as-is; no warranty given or implied.
; Credit welcome if you make something useful with this.
;
; Statements:
;   PRINT [item [; item ...]]
...
;
; v11.5 changes vs v11.4:
;   - Showcase: Mandelbrot now called via GOTO 600 (not GOSUB 600) with END at
;     line 740, matching the v11.2 structure that ran correctly in Kowalski.
;     Root cause: FOR loops inside GOSUB fail in Kowalski (NEXT cannot loop back
;     correctly). GOSUB/RETURN is still demonstrated via lines 360-400 / 500-530.
;   - Banner updated to v11.5.
;
...
INIT:
        ...
        LDX #$BF             ; clear ZP $00-$BF inclusive (includes DATA_PTR at $BC-$BD)
...
; =============================================================================
TKEMIT: LDY #0
        STA (T1),y           ; STA (zp) not supported; STA (zp),y with Y=0
        INC T1
        BNE TKEMIT_ok
        INC T1+1
TKEMIT_ok:
...
EL_found:
        LDA LP               ; save insertion point before DELINE corrupts LP
        PHA
        LDA LP+1
        PHA
        JSR DELINE           ; delete existing line (clobbers LP)
...
DP_expr:
        ...
        JSR WEAT             ; consume ')'
        BRA DP_top           ; loop back ? now print whatever follows AT(x,y)
DP_chk_chrs:
...
DP_semi:        JSR GETCI
        ...
        BEQ DP_ret
        JMP DP_top           ; more items: loop (BRA out of range, use JMP)
...
DO_GOTO:
        ...
        LDA T0+1
        STA CURLN+1
        JMP RUN_LINE         ; seek succeeded: unwind stack and run
...
; =============================================================================
; PUTSTR ? print a NUL-terminated string from ROM
...
DO_on_skip_tr:
        JMP DO_on_skip       ; hi byte set (>255 or negative): skip
DO_on_hi_ok:
...
DO_on_lp:
        ...
        JMP DO_on_skip       ; no comma: index out of range, fall through
DO_on_more:
        JSR GETCI            ; consume ','
        JMP DO_on_lp         ; loop (JMP; BRA out of range)
DO_on_go:
...
RD_f_go:
        ...
        LDA (DATA_PTR),y     ; Y=0: first body token
        CMP #TOK_DATA
        BEQ RD_found_data
...
;   Clobbers: A T0
; =============================================================================
E2_sgn: JSR GETCI            ; SGN(n)
        JSR EAT_EXPR
        JSR WEAT             ; consume ')'
        LDA T0
        ORA T0+1
        BEQ E2_sgn_zero      ; T0 == 0: return 0
        LDA T0+1
...
DO_let_dn:
        RTS
; =============================================================================
; PRT16 ? print T0 as a signed decimal integer (no leading zeros)
;   In:  T0   16-bit signed value
;   Out: decimal digits sent to PUTCH; '-' prefix if negative
;   Clobbers: A X T0 T1 T2
;   Method: repeated subtraction of TENS[] table entries, leading-zero
;   suppression via T2 flag (0 = suppress, $FF = printing has started).
;   Ones digit is always printed.
;   DANGER: TENS_LO / TENS_HI must be on the same ROM page as PRT16_sub
;   (they are placed immediately after to guarantee this).
; =============================================================================
PRT16:
        LDA T0+1
        BPL PRT16_pos
        LDA #'-'
        JSR PUTCH
        JSR NEG16            ; make positive
PRT16_pos:
        STZ T2               ; T2 = 0: suppress leading zeros
        LDX #4               ; start with ten-thousands place
PRT16_dl:
        LDA #'0'
        STA T1               ; current digit character
PRT16_sub:
        LDA T0
        CMP TENS_LO,x
        LDA T0+1
        SBC TENS_HI,x
        BCC PRT16_digit      ; T0 < TENS[x]: digit is done
        STA T0+1             ; T0 -= TENS[x]
        LDA T0
        SBC TENS_LO,x
        STA T0
...
PRT16_digit:
        LDA T1
        CPX #0               ; ones place? always print
        BEQ PRT16_put
        CMP #'0'
        BNE PRT16_start      ; non-zero digit: start printing
        BIT T2               ; bit 7 of T2 set = printing has started
        BMI PRT16_put
        BRA PRT16_next       ; suppress leading zero
PRT16_start:
        LDA #$FF
        STA T2               ; mark: printing has started
        LDA T1
PRT16_put:
        JSR PUTCH
PRT16_next:
...
; Constant table ? must remain on same ROM page as PRT16_sub
TENS_LO: .BYTE <1, <10, <100, <1000, <10000
TENS_HI: .BYTE >1, >10, >100, >1000, >10000
...
; =============================================================================
PEEKC:  LDA (IP)             ; 65C02 zp-indirect
        RTS
...
; PRNL ? print CR+LF, drops through into PUTCH for the LF (-3 bytes vs JMP PUTCH)
;   In:  ?
;   Out: ?
;   Clobbers: A
PRNL:   LDA #$0D
        STA IO_PUTCH         ; send CR directly, then fall through for LF
        LDA #$0A
...
; =============================================================================
; STRING TABLE  ?  placed after PUTCH, wherever the assembler puts it.
;   PUTSTR receives full 16-bit address in A (lo) / X (hi) ? no page constraint.
; =============================================================================
STR_BANNER: .DB "4K BASIC v11.6",$0D,$0A,0
STR_BYTES:  .DB " BYTES FREE",$0D,$0A,0
STR_ERROR:  .DB " ERR",0
STR_IN:     .DB " IN "
	brk	; so I can see the end of code
