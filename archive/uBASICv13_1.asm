; =============================================================================
; uBASIC v13.1  -  2 KB Tiny BASIC for the 65C02
; Pre-loaded program: feature showcase + Mandelbrot (type RUN to execute, NEW to clear)
; =============================================================================
; Copyright Vincent Crabtree, March 2026.
; Use as-is; no warranty given or implied.
; Credit to Oscar Toledo for his x86 BootBASIC inspiration.
; If you make money from this, congrats -- please credit me.
;
; Statements:   PRINT  IF  GOTO  LIST  RUN  NEW  INPUT  REM  END  LET
;               POKE   FREE   HELP
; Expressions:  + - * / %   =  <  >  <=  >=  <>   unary -
;               CHR$(n)   PEEK(addr)   USR(addr)   A-Z vars
; Numbers:      signed 16-bit  (-32768 .. 32767)
;
; Error codes (printed as "?N"):
;   ?0  SN  syntax / bad expression
;   ?1  UL  undefined line number
;   ?2  OV  division by zero (also modulo by zero)
;   ?3  OM  out of memory
;   ?4  UK  bad variable assignment
;
; ---- ROM memory map ---------------------------------------------------------
;   $F800          JMP INIT  (3-byte trampoline -- see Kowalski note)
;   $F803..$F864   string / keyword table  (all on page $F8)
;   $F865..$FFF8   interpreter code  (2041 bytes used, 7 bytes free)
;   $FFF9..$FFFB   free (3 bytes)
...
;   $04-$05  LP     line pointer / scratch pointer
;   $06-$07  T0     primary scratch / expression result
;   $08-$09  T1     secondary scratch
;   $0A-$0B  T2     tertiary scratch / STMT indirect jump target
;   $0C-$0D  CURLN  currently-executing line number
;   $0E      RUN    run flag: 0 = immediate mode, $FF = program running
;   $0F      OP     saved operator for MUL/DIV/MOD kernel ('*', '/', or '%')
;   $10-$2F  IBUF   input line buffer (32 bytes, indices 0-31)
;   $30-$4F  --     free scratch RAM
;   $50-$8B  VARS   A-Z variable store (2 bytes each, 52 bytes total)
;   $8C      RUNSP  stack-pointer snapshot for GOTO unwind
;
; ---- program storage --------------------------------------------------------
;   $0200 upward; hard ceiling RAM_TOP ($1000 for 4 KB SRAM)
;   Line format:  <lineno_lo> <lineno_hi> <raw ASCII body> <CR>
;   Body has no leading line-number text; token spaces are preserved.
;
; ---- Kowalski note ----------------------------------------------------------
;   Kowalski v2.x does not jump through the reset vector on startup; it
;   executes from the first byte of the assembled image.  A jump 
;   INIT trampoline at $F800 bridges over the string table so that both
;   Kowalski and real hardware (which uses the $FFFC vector) work correctly.
; =============================================================================
...
; ---- hardware constants ------------------------------------------------------
IO_OUT   = $E001             ; Kowalski UART output register
IO_IN    = $E004             ; Kowalski UART input register (0 = no char)

; ---- memory constants --------------------------------------------------------
RAM_TOP  = $1000             ; first address above usable SRAM

; ---- zero-page symbol definitions --------------------------------------------
IP       = $00               ; 16-bit: interpreter pointer
PE       = $02               ; 16-bit: program end (one past last byte)
LP       = $04               ; 16-bit: line pointer / scratch pointer
T0       = $06               ; 16-bit: primary scratch / expression result
T1       = $08               ; 16-bit: secondary scratch
T2       = $0A               ; 16-bit: tertiary scratch / STMT jump target
CURLN    = $0C               ; 16-bit: currently-executing line number
RUN      = $0E               ; 8-bit:  run flag (0 = immediate, $FF = running)
OP       = $0F               ; 8-bit:  saved operator for MUL/DIV/MOD kernel ('*', '/', or '%')
IBUF     = $10               ; 32-byte input line buffer
VARS     = $50               ; 52-byte variable store (A-Z, 2 bytes each)
RUNSP    = $8C               ; 8-bit:  stack pointer snapshot for GOTO unwind
...
ERR_OV   = 2                 ; division by zero
ERR_OM   = 3                 ; out of memory
ERR_UK   = 4                 ; bad variable assignment

; ---- misc constants ----------------------------------------------------------
IBUF_MAX = 31                ; maximum index into IBUF (buffer holds 32 bytes)
VARS_MAX = $3B               ; highest X index used when clearing VARS ($50..$8B)
...
; Kowalski trampoline (see note in file header).
; Real hardware reaches INIT via the $FFFC reset vector instead.
         BRA INIT

; =============================================================================
; STRING / KEYWORD TABLE  (page $F8, starts at $F802)
;
; PUTSTR and MTCHKW address strings using STR_PAGE as the hi-byte, so every
; entry in this table MUST remain on the same page as STR_BANNER.
; Current usage: ~$4A bytes.  Do not let T_END cross a page boundary.
;
; Layout:  human-readable strings first, then keyword tokens.
; Keyword strings are NUL-terminated so MTCHKW knows where each ends.
; KW_CHRS has no NUL -- MTCHKW stops at the sentinel byte T_END (BRK/$00).
; =============================================================================
STR_PAGE  = >STR_BANNER      ; hi-byte shared by all string/kw addresses

STR_BANNER: .DB "uBASIC v13.1"  ; startup banner (NUL-free; printed by length via STR_CRLF)
STR_CRLF:   .DB CR,LF,0       ; CR+LF terminator string
STR_IN:     .DB " IN ",0      ; used by DO_ERROR: "?N IN <line>"
...
KW_LET:     .DB "LET",0
KW_THEN:    .DB "THEN",0
KW_CHRS:    .DB "CHR$",0     ; Opening paren consumed by EAT_EXPR;
KW_POKE:    .DB "POKE",0
KW_FREE:    .DB "FREE",0      ; also used as display string by DO_FREE (saves a separate STR_FREE)
KW_HELP:    .DB "HELP",0
KW_PEEK:    .DB "PEEK",0
KW_USR:     .DB "USR",0
KW_LIST_END:.DB 0              ; double-NUL sentinel: marks end of keyword list for DO_HELP

; =============================================================================
; INIT  -  cold start
;
;   In:  --  (entered via reset vector or Kowalski BRA trampoline)
;   Out: never returns; falls through to MAIN
;   Clobbers: everything
; =============================================================================
INIT:
         LDX #$FF
         ...
         LDX #RUNSP           ; zero ZP from $00 to $8C inclusive
INIT_Z:  STZ 0,X              ; 65C02: STZ zp,X  (2 bytes, no LDA #0 needed)
         DEX
         BPL INIT_Z
         ...
         STA PE+1
         LDA #<STR_BANNER
         JSR PUTSTR           ; print banner + CR/LF (STR_CRLF immediately follows)
...
;
;   Reads one line.  Lines beginning with a digit are routed to EDITLN
;   (program store edit); all others are executed immediately via STMT.
; =============================================================================
MAIN:
         STZ RUN
         JSR GETLINE_M        ; print "> "; read line into IBUF; IP = IBUF
         JSR WPEEK            ; skip spaces; peek first non-space char
         CMP #CR
         BEQ MAIN             ; blank line -- reprompt
         CMP #'0'
         BCC MAIN_DIR         ; below '0' -- not a digit
         CMP #'9'+1
         BCS MAIN_DIR         ; above '9' -- not a digit
         JSR EDITLN           ; starts with digit: edit program store
         BRA MAIN
MAIN_DIR:
         JSR STMT_LINE         ; execute statement(s), honouring ':' separator
         BRA MAIN
...
;   Out: variable updated; IP restored to source position after var name
;   Clobbers: A X Y T0 T1 T2 IP (temporarily)
; =============================================================================
DO_INPUT:
...
;     GETLINE    no prompt (unused externally but structurally clean)
;
;   In:  --
;   Out: IBUF filled, terminated with CR; IP = IBUF
;   Clobbers: A X IP
;
;   Trick: GETLINE_M loads '>' then uses a BIT abs opcode ($2C) as a
;   2-byte no-op that skips the following LDA #'?' instruction.
; =============================================================================
GETLINE_M:
         LDA #'>'
...
PNUM:
         ...
         STZ T0               ; clear result
         STZ T0+1
PN_LP:   LDA (IP)             ; 65C02 zp-indirect: peek without consuming
         SEC
...
EL_LEN:  LDA (LP),Y
         ...
         INC LP+1
         BRA EL_FL
EL_FND:  LDA LP               ; save insertion point before DELINE corrupts LP
         PHA
         LDA LP+1
         PHA
         JSR DELINE            ; delete existing line (clobbers LP via shift loop)
         PLA                  ; restore LP to original insertion point
         STA LP+1
         PLA
         STA LP
EL_INS:  JSR WSKIP             ; skip spaces between line number and body
         JSR PEEKC
         CMP #CR
         BEQ EL_DN             ; empty body: delete only (EL_DN = DL_OK = RTS above)
         CMP #0
         BEQ EL_DN
...
DP_AFT:  JSR WPEEK
         ...
         BRA DP_TOP
DP_NL:   LDA #<STR_CRLF
         ; fall through into PUTSTR -- prints CR/LF then returns to caller
...
;
;   DO_END and RUNEND are adjacent: DO_END is the STMT handler; RUNEND is the
;   internal label used when the program runs off the end or RUN is cleared.
;   Both need only STZ RUN / RTS.
; =============================================================================
DO_END:
RUNEND:  STZ RUN
         RTS
...
DO_NEW:
         ...
         STA PE+1
         LDX #VARS_MAX
DO_NWZ:  STZ VARS,X           ; 65C02: STZ zp,X (2 bytes; no LDA #0 needed)
         DEX
         BPL DO_NWZ
         RTS
...
DO_POKE:
         ...
         LDA T0                ; save address in T1
         STA T1
         LDA T0+1
         STA T1+1
         JSR WEAT              ; skip spaces, consume ','
         JSR EXPR              ; evaluate value -> T0
         LDA T0                ; value byte (lo only)
         LDY #0
         STA (T1),Y            ; write to address (T1 is ZP pointer $08/$09)
         RTS
...
;   Out: decimal count followed by " FREE" printed to terminal
;   Clobbers: A T0
;
;   Computes RAM_TOP - PE as a 16-bit subtraction, prints via PRT16, emits a
;   literal space, then tail-calls PUTSTR(KW_FREE) -- reusing the "FREE" keyword
;   string avoids a separate STR_FREE entry.
; =============================================================================
DO_FREE:
         SEC
         LDA #<RAM_TOP
         ...
         LDA #<KW_FREE
         JMP PUTSTR            ; tail call: reuses KW_FREE = "FREE",0

; =============================================================================
; DO_HELP  -  HELP  :  print keyword list to terminal, one per line
;
;   In:  --
;   Out: all keywords from KW_PRINT to KW_LIST_END printed, one per CR+LF line
;   Clobbers: A Y T2
;
;   Walks the contiguous keyword string block (KW_PRINT .. KW_LIST_END) using
;   T2 as a ZP pointer and Y as an offset.  On each NUL (end of a keyword)
;   emits CR+LF; a second consecutive NUL (KW_LIST_END) terminates the walk.
; =============================================================================
DO_HELP:
         LDA #<KW_PRINT
         STA T2
         ...
         STA T2+1
         LDY #0
DH_LP:   LDA (T2),Y           ; load next char
         BEQ DH_NUL            ; NUL: end of this keyword
         JSR PUTCH             ; print keyword char
         INY
         BRA DH_LP
DH_NUL:  INY                   ; step past the NUL
         LDA (T2),Y            ; peek at next char
         PHA                   ; save it
         LDA #CR               ; emit CR+LF after each keyword
         JSR PUTCH
         LDA #LF
         JSR PUTCH
         PLA                   ; restore peeked char
         BEQ DH_DN             ; second NUL = end of list
         BRA DH_LP
DH_DN:   RTS
...
;
;   In:  A = ERR_xx error code (0-4)
;   Out: never returns to caller; jumps to MAIN
;   Clobbers: everything
;
;   Entry from E1_OVFL is a fall-through (saves 3 bytes vs JMP DO_ERROR).
...
DO_ERROR:
         ...
         JSR PUTCH            ; print "?N"
         LDA RUN
         BEQ DO_ERR_NL        ; not running: skip " IN <line>" annotation
         LDA #<STR_IN
         JSR PUTSTR
         LDA CURLN
         STA T0
...
E2_NOT_PAR:
         ...
         BRA WEAT             ; tail call: consume ')' and return (within BRA range)
E2_NOTCHRS:
         LDA #<KW_PEEK
...
E2_BAD:  STZ T0               ; unrecognised atom: return 0 (no error)
         STZ T0+1
         RTS
...
;   Clobbers: A IP
; =============================================================================
GETCI:   LDA (IP)             ; 65C02 zp-indirect (2 bytes, no Y needed)
...
STLN_CHK:
         ...
         JSR GETCI             ; consume ':'
         BRA STMT_LINE         ; execute next statement
STLN_RTS: RTS
...
WSKIP_NS:
WSKIP:
WPEEK:   LDA (IP)             ; 65C02 zp-indirect peek (no Y needed)
         CMP #' '
...
; =============================================================================
; PEEKC  -  peek char at IP without advancing
;
;   In:  IP -> char to inspect
;   Out: A = char at IP; IP unchanged
;   Clobbers: A
; =============================================================================
PEEKC:   LDA (IP)
         RTS
...
GETCH:   LDA IO_IN
         ...
         BRA PUTCH            ; echo it, then return (tail call)
...
MTCHKW:
         ...
         LDA IP+1
         STA LP+1
         JSR WSKIP_NS         ; skip leading spaces (does not update LP)
MK_LP:   LDY #0
         LDA (T1),Y           ; fetch next keyword char (NUL = end of keyword)
         BEQ MK_OK            ; keyword exhausted: full match
         JSR PEEKC            ; peek input char (Y still 0)
         JSR UC               ; uppercase for comparison
         CMP (T1),Y
         BNE MK_FAIL          ; mismatch
         JSR GETCI            ; consume matched char
         INC T1               ; advance keyword pointer (16-bit)
         BNE MK_LP
         INC T1+1
         BRA MK_LP
MK_OK:   CLC
         RTS
MK_FAIL: LDA LP               ; restore IP
         STA IP
         LDA LP+1
         STA IP+1
         SEC
...
; Feature showcase program exercising major uBASIC v13.1 capabilities.
...
;
; === ROM / real-hardware note ===
; When running from ROM (no pre-loaded program), INIT must NOT set PE to
; point past a program.  Change the two lines in INIT that read:
;   LDA #<SHOWCASE_END  ->  LDA #<PROG
;   LDA #>SHOWCASE_END  ->  LDA #>PROG
; This sets PE = PROG = $0200 so the interpreter starts with an empty program.
;
         .DB $0A,$00,$52,$45,$4D,$20,$75,$42,$41,$53,$49,$43,$20,$76,$31,$33,$20,$2D,$20,$53,$48,$4F,$57,$43,$41,$53,$45,$0D  ; 10 REM uBASIC v13.1 - SHOWCASE
         .DB $14,$00,$50,$52,$49,$4E,$54,$20,$22,$2D,$2D,$20,$75,$42,$41,$53,$49,$43,$20,$76,$31,$33,$20,$53,$48,$4F,$57,$43,$41,$53,$45,$20,$2D,$2D,$22,$0D  ; 20 PRINT "-- uBASIC v13.1 SHOWCASE --"
