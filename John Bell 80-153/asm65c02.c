 ...
 * v1.7: Conventional sidecar .LST listing files are generated by default.
 *       Use -NoList to suppress list-file generation.
 * v1.8: Four correctness fixes found while porting a 4KB float BASIC:
 *       (1) "INC A" / "DEC A" with an explicit "A" operand were silently
 *           mis-assembled as zero-page ($1A/$3A "inc a" was only ever
 *           recognized when the operand was *completely absent*, e.g.
 *           bare "INC"). A single-character "A"/"a" operand on an
 *           accumulator-capable mnemonic (asl/lsr/rol/ror/inc/dec) is now
 *           also recognized as M_ACC, in both pass 1 and pass 2.
 *       (2) has_undef() -- used on pass 1 to decide whether an operand
 *           contains a forward-referenced symbol -- scanned for any
 *           isalpha() run without first skipping past $hex/%%binary/decimal
 *           literals, so the hex digits A-F inside a literal like "$AF"
 *           were misread as one-character symbol references and reported
 *           undefined. This forced 3-byte M_ABS sizing on pass 1 while
 *           pass 2 correctly resolved the literal to 2-byte M_ZP, leaving
 *           a stray unwritten (zero) byte after every such instruction.
 *           has_undef() now skips $/%%/decimal literals exactly as
 *           eval_expr() does, since it exists purely to predict
 *           eval_expr()'s pass-2 outcome.
 *       (3) Address/pc values were inconsistently masked to 16 bits:
 *           label addresses were stored in the symbol table completely
 *           unmasked (sym_set(full, pc) instead of pc & 0xFFFF), while
 *           .byte emission masked correctly but .word emission and
 *           instruction-byte emission wrote directly through mem[pc],
 *           mem[wpc] etc. with NO masking at all -- a genuine
 *           out-of-bounds write into the fixed uint8_t mem[65536] array
 *           once accumulated pc exceeded 65536, not merely a cosmetic
 *           reporting bug. There was also no diagnostic anywhere for this
 *           condition; a wrapped/corrupted image previously reported
 *           "No errors." All four emission/label-definition sites now
 *           mask consistently and call check_pc_overflow() with the true
 *           pre-mask address; assemble() now folds any overflow into the
 *           normal error list (hard failure, non-zero exit, no output
 *           file written) instead of silently succeeding.
 *       (4) derive_lst_path() compared two strrchr() results (slash1,
 *           slash2) with a bare "slash1 > slash2", which is undefined
 *           behavior in C whenever either pointer is NULL -- reachable on
 *           ordinary inputs like a bare filename with no path separator
 *           at all. The three NULL/non-NULL cases are now handled
 *           explicitly before any pointer comparison is performed.
 * v1.9 (audit fix pass + syntax-test regression fixes, Jul 2026):
 *       (1) New M_IND_X addressing mode: (zp,X) indexed indirect, 2 bytes.
 *           Previously entirely unparsed -- "(expr,X)" fell through to the
 *           bare-"(expr)" case and was silently mis-assembled. Now wired
 *           into adc/and/cmp/eor/lda/ora/sbc/sta ($x1 opcodes). This mode
 *           is NOT 65C02-only -- it exists on NMOS 6502 too.
 *       (2) New M_IND_ABSX addressing mode: JMP (abs,X), 3 bytes, $7C.
 *           Previously a no-op stub (see old is_65c02only() comment).
 *           65C02-only; distinguished from M_IND_X by mnemonic (jmp) in
 *           parse_operand(), since both share "(...,X)" source syntax but
 *           differ in operand width and legal mnemonics.
 *       (3) Missing abs,X/abs,Y opcode rows added: adc/cmp/sbc abs,Y;
 *           and/eor/ora abs,X and abs,Y. All were legal 6502/65C02
 *           opcodes with no OPTAB entry.
 *       (4) tsb/trb wired into OPTAB. They were already listed in
 *           is_65c02only()'s 65C02-only mnemonic list and in --help, but
 *           had no opcode-table rows at all -- never actually assemblable.
 *       (5) stz abs,X ($9E) added -- same category of gap as (4).
 *       (6) Default-mode 65C02-instruction warnings: assembling a
 *           65C02-only instruction while NOT under .opt proc6502 now
 *           produces a warning by default (new warnings[] list, printed
 *           alongside errors), since the code may be intended to also run
 *           on NMOS 6502 hardware. New CLI flags:
 *             -NoWarn65c02   suppress these warnings
 *             -Strict6502    promote them to hard errors instead (wins
 *                            over -NoWarn65c02 if both given); equivalent
 *                            to a global .opt proc6502 without editing
 *                            the source. Unrelated to and does not change
 *                            the pre-existing .opt proc6502 hard-error
 *                            behavior, which still always applies.
 *       (7) Bug fix: on assembly error, the human-readable report used to
 *           print "Key symbols:", the reset vector, --dump-all output, and
 *           the ROM-footprint size_report() unconditionally, even though
 *           mem[]/syms[] may be incomplete or reflect a truncated pass 2.
 *           That whole block is now gated on successful assembly (ok==1).
 *       (8) BBR0-7/BBS0-7 (Rockwell/WDC 65C02 bit-branch, two operands:
 *           zp,target; always 3 bytes) and RMB0-7/SMB0-7 (single zp
 *           operand, 2 bytes) added. RMB/SMB fit the existing generic
 *           single-operand OPTAB model directly; BBRn/BBSn are handled as
 *           a self-contained special case in pass 1/2 (see is_bbr_bbs())
 *           since their two-operand shape doesn't fit Operand/
 *           parse_operand(). Sim65c02.c already executed all of these.
 *       (10) cpu_mode is now tri-state (0=unspecified/default,
 *           1=explicit .opt proc6502, 2=explicit .opt proc65c02) instead
 *           of boolean. .opt proc6502 now implies the same intent as
 *           -Strict6502; .opt proc65c02 implies the same intent as
 *           -NoWarn65c02. It is a hard error for a directive's implied
 *           policy to contradict an explicit, opposite CLI flag (checked
 *           at the directive's line, not silently resolved), and -NoWarn
 *           65c02+-Strict6502 together on the command line is rejected
 *           before assembly starts. Switching between proc6502/proc65c02
 *           multiple times within one file (mixed-target sections) is
 *           still fully supported and unaffected. Bug fix alongside this:
 *           pass 2 now resets cpu_mode to 0 before it starts, instead of
 *           inheriting whatever value pass 1 ended on -- previously a
 *           late .opt proc6502 near the end of a file could leave pass 2
 *           starting with cpu_mode==1 for lines *before* pass 2 itself
 *           reached that directive. sim65c02.c's embedded assemble()
 *           call is unaffected (no CLI flags passed, so cpu_mode==0 by
 *           default unless the source itself sets it).
 *       (9) .INCLUDE "file.asm" (recursive, depth-capped, circular-guard,
 *           flattened before pass 1 with correct per-file line-number
 *           tagging in errors/warnings -- see expand_includes()) and
 *           .INCBIN "file.bin" (raw bytes emitted through the standard
 *           overflow/masking/listing machinery, like .RES/.DB) added.
 *       (11) Bug fix: ZP/ABS mode selection was decided purely by the
 *           numeric VALUE of a hex literal, ignoring how many digits were
 *           written -- "$0000"/"$00FF"-style explicit-width literals
 *           silently assembled as 2-byte zero-page instead of the 3-byte
 *           absolute form their digit count implies. Fixed via
 *           literal_forces_abs(): a bare ">=3-hex-digit" literal now
 *           forces absolute width. Found via a syntax-test file
 *           exercising "ORA $0000" and similar.
 *       (12) That fix exposed a second, previously-unreachable pre-
 *           existing gap: ASL/LSR/ROL/ROR/DEC/INC/LDY abs,X had NO OPTAB
 *           rows at all (masked because "$0000,X"-style operands always
 *           silently fell back to the ,X zero-page opcode before fix 11).
 *           Added all seven, plus the four of those (LSR/ROL/ROR/LDY
 *           abs,X) sim65c02.c didn't execute yet either.
 *       (13) Bug fix: LDX's indexed zero-page OPTAB row was tagged
 *           M_ZPX with opcode $96 -- but LDX has no zero-page,X form on
 *           real 6502/65C02 (only zero-page,Y, $B6), and $96 is actually
 *           STX's zero-page,Y opcode. This silently mis-assembled
 *           "LDX zp,Y" as a spurious promotion to 3-byte absolute,Y
 *           instead of the correct 2-byte $B6 form. sim65c02.c already
 *           executed $B6/$96 correctly for LDX/STX respectively -- purely
 ...
 *   v1.7  (Mar 2026)  Generate conventional .LST sidecar listings by default;
 *                     -NoList suppresses listing generation.
 */
