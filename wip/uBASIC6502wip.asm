; =============================================================================
; JB-uBASIC6502 v1.13  --  2 KB Tiny BASIC (NMOS 6502) for John Bell 80-153 SBC
...
;
; ---- version lineage --------------------------------------------------------
;   V1.13 (Aug 2026)  31 bytes free before vectors. Added $-prefixed hex
;                     literals (e.g. $BEEF), unified into PNUM as a single
;                     PNUM/PHEX radix-parameterized routine (T1 holds the
;                     radix, 10 or 16) rather than a separate hex parser --
;                     this alone made the feature 25 bytes cheaper than the
;                     first (separate-routine) attempt, which needed 57
;                     bytes and didn't fit; the unified version costs 32.
;                     WSKIP call removed from PNUM/PHEX: verified redundant
;                     by tracing both callers (EDITLN via MAIN's WPEEK,
;                     E2_LIT via EXPR2's WPEEK) -- both already guarantee
;                     leading spaces are skipped before reaching here.
;                     MATCH_DISPATCH: MD_NOPAREN and MD_FAIL's push/RTS
;                     sequences folded into one shared block via an INX on
;                     the matched-entry path only (shifts which table bytes
;                     +1/+2 read to line up with the sentinel's own layout).
;                     Saves 8 bytes. Needed a long-branch fix (BEQ E2_NEG ->
;                     BNE/JMP) since combined PNUM/PHEX pushed that branch
;                     out of short-branch range -- caught by the assembler,
;                     not silently miscompiled.
;                     EXPR's RL_TEST: dropped a BIT-trick that existed only
;                     to skip a redundant LDA #0 -- A is already 0 from the
;                     preceding AND on the false path, so REL_F can just be
;                     the true-path's fallthrough target instead. Simpler
;                     AND smaller (3 bytes) than the trick it replaced.
;                     Evaluated but did NOT apply: (a) uppercasing on
;                     GETLINE ingestion to drop WPEEK_UC/UC/UCIP -- confirmed
;                     regression, corrupts case in PRINT "..." strings and
;                     REM comments (tested: "Hello World" -> "HELLO WORLD");
;                     (b) generic PE_CMP/T0_CMP_LP compare via CMP zp,Y --
;                     not a real 6502 addressing mode (only LDX/STX get
;                     zp,Y), assembles as 3-byte CMP abs,Y instead of the
;                     assumed 2-byte form, making it a net +5 bytes, not a
;                     saving.
;                     All of the above verified via sim65c02 (KOWALSKI=1):
;                     full prior regression suite plus new coverage for all
;                     6 relational operators (both outcomes), hex literals,
;                     decimal edge case ("10A" correctly stops at 'A'),
;                     implicit/explicit LET, and ABS/PEEK/POKE's paren-eating
;                     MATCH_DISPATCH path.
;
;                     Added NOT(x) (bitwise complement), 1-arg function
;                     reusing MATCH_DISPATCH's existing EAT_PAREN protocol --
;                     T0 already holds the argument on entry, same as
;                     ABS/PEEK. 17 bytes (4-byte table entry + 13-byte
;                     handler: EOR #$FF on each byte of T0). Evaluated but
;                     did NOT add AND/OR/XOR at that point: a 2-arg function
;                     needs either a dedicated "eat (, parse a, eat comma,
;                     parse b, eat )" prefix (~34 bytes for one op alone) or
;                     a trick reusing EAT_PAREN's non-validating single-char
;                     eat to swallow the comma as if it were the closing
;                     paren, then have the handler parse the second arg and
;                     eat the real ')' itself (cheaper: ~31 bytes for one
;                     2-arg op alone).
;
;                     Added AND(a,b)/OR(a,b)/XOR(a,b) on top of NOT, on
;                     request, accepting real-ROM overflow (~60-65 bytes
;                     over budget) so the logic could be verified via
;                     sim65c02 before a later hand-optimization pass trims
;                     it to fit. Uses the EAT_PAREN-comma-reuse trick above,
;                     with a shared BITOP2_PREFIX for the second-arg parse.
;
;                     IMPORTANT CORRECTION to something said earlier this
;                     session: KOWALSKI does NOT leave the ROM size budget
;                     unaffected. The interpreter is placed at .ORG $F800
;                     either way, but KOWALSKI=1 swaps in a few-instruction
;                     memory-mapped GETCH/PUTCH, while the real KOWALSKI=0
;                     build uses full bitbang-serial UART code -- much
;                     bigger. A build that assembles clean under KOWALSKI=1
;                     is NOT proof it fits the real ROM; verifying an
;                     over-budget feature requires either accepting the
;                     assembler's total refusal to produce output (no
;                     partial image on overflow, in EITHER mode -- confirmed
;                     by testing) or temporarily removing other, unrelated
;                     code to get back under budget for a real test run,
;                     which is what was done here: DO_INPUT/KW_INPUT and
;                     DO_POKE/KW_POKE were temporarily stripped in a
;                     throwaway test copy (not this file) to make room,
;                     verified there, then the identical fix applied here.
;
;                     Two real bugs found ONLY by simulating, not by
;                     re-reading the code -- both documented in full at
;                     BITOP2_PREFIX's own header, summary here:
;                     (1) First arg was saved into T1 directly, but T1 is
;                     also PNUM's radix scratch (from the hex-literal work
;                     above) -- the second arg's nested EXPR->PNUM call
;                     clobbered it before the combine ran. Every result
;                     came out equal to some function of the radix (10 or
;                     16) instead of the actual first argument -- e.g.
;                     AND(12,10) returned 10, not 8. Fixed by preserving
;                     the first arg on the hardware stack instead (same
;                     technique EXPR_ADD already uses for its own left
;                     operand) until after the second EXPR call returns.
;                     (2) BITOP2_PREFIX originally fell through into
;                     DO_AND's combine+RTS to save 3 bytes on AND's case.
;                     But DO_OR/DO_XOR reach it via JSR -- a fallthrough
;                     into DO_AND meant that JSR silently ran DO_AND's
;                     entire body (RTS included) before returning, so T0
;                     was already AND-combined by the time DO_OR's own ORA
;                     ran -- e.g. OR(12,10) returned 12 (the AND result,
;                     OR'd again), not 14. Fixed: BITOP2_PREFIX now has its
;                     own RTS; all three callers pay the same JSR, no free
;                     fallthrough for any of them.
;                     Verified (in the trimmed test copy, then confirmed
;                     identical on this file's own fix): AND(12,10)=8,
;                     OR(12,10)=14, XOR(12,10)=6, AND($FF,$0F)=15,
;                     OR($F0,$0F)=255, XOR($FF,$FF)=0, AND(0,255)=0,
;                     OR(0,0)=0, NOT(AND(5,3))=-2, XOR(XOR(5,3),3)=5,
;                     AND(65535,1)=1 -- all hand-checked against expected
;                     values, all correct. Full prior regression suite
;                     (minus INPUT/POKE-dependent cases, stripped in the
;                     test copy) also confirmed unaffected.
...
; Last byte of each string has bit 7 set; PUTSTR masks it before printing.
        .IF KOWALSKI
STR_BANNER: .DB "uBASIC6502 v1.13"  ; startup banner
        .ELSE
STR_BANNER: .DB "JB uBASIC v1.13"  ; startup banner
        .ENDIF
...
DL_LL:   LDA (LP),Y           ; scan body + CR
         ...
         LDA LP
         STA T1                ; T1 = destination pointer, starts at LP
         LDA LP+1
         ADC #0
         STA T0+1
         LDA LP+1
...
EL_INS:  JSR WPEEK             ; skip spaces + peek (no consume) first body char
         ...
         BEQ EL_DN             ; CR only: delete-only (no body to insert) --
                                ; EL_DN is at INSLINE's end (see below)
...
GT_OK:   LDA T0               ; T0 already == the matched line number
         ...
         LDA T0+1
         STA CURLN+1
         JSR ADD2_LP           ; LP += 2, past the 2-byte header (shared helper;
                                ; caller doesn't need LP preserved -- see header)
         LDA LP                ; IP = LP (now at body)
         STA IP
         LDA LP+1
         STA IP+1
GT_R:    CLC
...
;   EAT_PAREN protocol as a trick: EAT_PAREN eats '(' + parses the first
;   expr into T0 + blindly eats ONE more character (WEAT doesn't validate
;   what it eats -- for a real 1-arg function that's the closing ')'; here
;   it's actually the ',' separating the two arguments). BITOP2_PREFIX then
...
;   (each LDX #imm is a genuine 2-byte immediate load, so the skip math
;   here is fine -- unlike the bug below, which was about indexed zero page,
;   not immediate). Op selector ends up in Y for the combine loop.
;
...
;   -- T0,Y does NOT (6502 only gives LDX/STX a zero-page,Y mode; every
;   other opcode indexed by Y needs 3-byte absolute,Y). A first version of
;   this used X as the op selector and Y as the byte offset -- the
;   .DB $2C skips assumed exactly 2 bytes, but "AND T0,Y" assembles as 3
;   (39 00 00, confirmed against the real assembler output), so the skip
;   landed on the stray third byte of the "skipped" instruction, which
;   happened to be $00 -- BRK. Confirmed by direct disassembly before
;   writing a single line of the fix, not by running it and seeing wrong
;   answers.
; =============================================================================
